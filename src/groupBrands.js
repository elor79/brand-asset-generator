...
export const NEOORTHO_MARK = {
  ...
  glyph: { x: 184.38, y: 58.07, w: 1904.49, h: 450.07 },
  paths: [
...
export const KERIMEDICAL_MARK = {
  ...
  // GLYPH IS THE BRAND ONLY — measured from `paths`, which excludes the byline.
  // check_group re-measures it, so this cannot quietly drift.
  glyph: { x: 0.567, y: 0.566, w: 292.938, h: 85.839 },
  paths: [
...
// ─── Negative / mono variants ─────────────────────────────────────────────────
// A logo on a saturated gradient cannot keep its own colours: NeoOrtho's violet on
// the Group's blue end is nearly invisible. Every sub-brand ships a negative in the
// supplied artwork for exactly this reason, so `variant` is honoured, never guessed.

/**
 * KeriMedical as it appears ALONE — byline included, because away from the Group
 * mark the byline is the only thing saying whose company this is.
 */
export const KERIMEDICAL_FULL = {
  view: KERIMEDICAL_MARK.view,
  glyph: { x: 0.434, y: 0.566, w: 293.071, h: 113.527 },
  paths: [...KERIMEDICAL_MARK.byline, ...KERIMEDICAL_MARK.paths],
};

export const MARK_VARIANTS = ['color', 'white', 'mono'];

/** The paths of a mark, recoloured for the surface it lands on. */
export function markPaths(mark, variant, inkColor) {
  if (!mark) return [];
